...
Cat.train.oc <- Cat.train %>% 
  dplyr::select(-c("Func_group_cat", "Name", "ChemFormula", "Func_Group_Cat_3", "Haofei_cat", "Cstar_Cat", "RT1", "Retention_index"))
...
for(i in 1:nrow(Cat.test.form)){
  
  #i = 10
  form.t <- Cat.test.form$ChemFormula[i]
  ...
  Cat.test.form$Cnum[i]= form.t4$C[1]
  Cat.test.form$Hnum[i]= form.t4$H[1]
  Cat.test.form$Onum[i]= form.t4$O[1]
  Cat.test.form$Nnum[i]= form.t4$N[1]
  Cat.test.form$Snum[i]= form.t4$S[1]
}



Cstar_params <- read_csv("C_star_params.csv")
Cat.train.form <- Cat.train.form %>% 
  left_join(Cstar_params) %>% 
  mutate(cstar = (n_0_c-Cnum)*b_c-(Onum*b_O)-2*((Cnum*Onum)/(Cnum+Onum))*b_CO-Nnum*b_N)

Cat.train.cs <- Cat.train.form %>% 
  dplyr::select(-c("Func_group_cat", "Name", "ChemFormula", "Func_Group_Cat_3", "Haofei_cat", "Cstar_Cat", "O_C_ratio", "RT1", "RI.round", "RI.round.f", "Retention_index", "Cnum", "Hnum", "Onum",
                   "Nnum", "Snum", "n_0_c", "b_c", "b_O", "b_CO", "b_N"))

Cat.test.form <- Cat.test.form %>% 
  left_join(Cstar_params) %>% 
  mutate(cstar = (n_0_c-Cnum)*b_c-(Onum*b_O)-2*((Cnum*Onum)/(Cnum+Onum))*b_CO-Nnum*b_N)

Cat.test.cs <- Cat.test.form %>% 
  dplyr::select(-c("Func_group_cat", "Name", "ChemFormula", "Func_Group_Cat_3", "Haofei_cat", "Cstar_Cat", "O_C_ratio", "RT1", "RI.round", "RI.round.f", "Retention_index", "Cnum", "Hnum", "Onum",
                   "Nnum", "Snum", "n_0_c", "b_c", "b_O", "b_CO", "b_N"))
```
...
```{r}
set.seed(144)
train.rf = train(cstar ~., data = Cat.train.cs, method = "rf", tuneGrid = data.frame(mtry=seq(1, 45, 1)), trControl = trainControl(method = "cv", number = 5), metric = "RMSE", na.action = na.exclude)
...
Cat.mm = as.data.frame(model.matrix(cstar ~., data = Cat.test.cs))
...
Cat.test.cs$cstar.p <- pred.best.rf

Cat.test.cs.2 <- Cat.test.cs %>% 
  mutate(pct_error_cs = 100*(abs(cstar.p-cstar)/cstar))
summary(Cat.test.cs$pct_error_oc)

Cat.test.cs.2$Name <- Cat.test$Name

#OSR2(pred.best.rf, Cat.train.cs$cstar, Cat.test.cs$cstar)

Cat.test.cs %>% 
  ggplot(aes(x = cstar, y = cstar.p))+
  geom_point()+
  xlab("log cstar")+
  ylab("log cstar predicted")




```
Predict ch cho chon chons

use volatility basis set- Donahue 2012
...
line <- data.frame(x = c(0, 15000), y = c(0, 15000))

rf_pcterr_comparison %>% 
  ggplot(aes(x = slopemod.test.slope.i, y = pred.best.rf.s))+
  geom_point(color = "blue")+
  geom_point(aes(x= slopemod.test.slope.i, y = slope_fromclosest.i.p))+
