...
IS_pos_short_dalk <- IS_positions_avg %>% 
  filter(startsWith(Compound.Name, 'dC'))%>% 
  select(relative_LRI, relative_LRI2)

IS_positions_avg_dalk <- IS_positions_avg %>% 
  filter(startsWith(Compound.Name, 'dC'))
...
testnn_dalk <- nn2(IS_pos_short_dalk, query = Analyte_pos_short, treetype = "kd", searchtype = "radius", k=1, radius = 2)

Analyte_IS_Index <- data.frame(testnn$nn.idx)

Analyte_IS_Index_dalk <- data.frame(testnn_dalk$nn.idx)

Analyte_pos_ad <- data.frame(Compound.Name = Analyte_positions$Compound.Name, IS_index1 = Analyte_IS_Index$X1, IS_index2 = Analyte_IS_Index$X2, IS_index3 = Analyte_IS_Index$X3, IS_index_dalk = Analyte_IS_Index_dalk$testnn_dalk.nn.idx)

Manual_IS_assignments <- read_csv("Manual_IS_assignments.csv")
...
Analyte_with_IS <- Analyte_pos_ad %>% 
  ...
  mutate(IS_appropriate_dalk =IS_positions_avg_dalk$Compound.Name[IS_index_dalk]) %>% 
  left_join(Manual_IS_assignments)
...
Analyte_with_IS_positions <- Analyte_with_IS %>% 
  ...
  left_join(IS_positions_avg, by = c("IS_appropriate_dalk"= "Compound.Name")) %>% left_join(IS_positions_avg, by = c("Manual_IS"= "Compound.Name"))
...
M_t_analyte_withIS <- M_t_analyte_unique %>% 
  ...
  mutate(IS_vol_man= -999*(LRI.I/LRI.I)) %>% 
  mutate(IS_mean_norm_man= -999*(LRI.I/LRI.I)) 
...
for(i in 1:nrow(M_t_analyte_withIS)){
  ...
  IS_name_dalk = M_t_analyte_withIS$IS_appropriate_dalk[i]
  IS_name_man = M_t_analyte_withIS$Manual_IS[i]
  ...
  IS_indicated_dalk <- IS_stability %>% 
    filter(Compound.Name == IS_name_dalk) %>% 
    filter(File_num == File_num_t)
  
  M_t_analyte_withIS$IS_vol_dalk[i] <-IS_indicated_dalk$Volume_tot[1]
  M_t_analyte_withIS$IS_mean_norm_dalk[i] <-IS_indicated_dalk$IS_mean_norm[1]
  
  IS_indicated_man <- IS_stability %>% 
    filter(Compound.Name == IS_name_man) %>% 
    filter(File_num == File_num_t)
  
  M_t_analyte_withIS$IS_vol_man[i] <-IS_indicated_man$Volume_tot[1]
  M_t_analyte_withIS$IS_mean_norm_man[i] <-IS_indicated_man$IS_mean_norm[1]
...
M_t_analyte_withIS <- M_t_analyte_withIS %>% 
  ...
  mutate(vol_is_normnorm_1 = Volume/IS_mean_norm1) %>% 
  mutate(vol_is_rawnorm_1 = Volume/IS_vol1) %>% 
  mutate(vol_is_normnorm_dalk = Volume/IS_mean_norm_dalk) %>% 
  mutate(vol_is_rawnorm_dalk = Volume/IS_vol_dalk) %>% 
  mutate(vol_is_normnorm_man = Volume/IS_mean_norm_man) %>% 
  mutate(vol_is_rawnorm_man = Volume/IS_vol_man)  
```
...
cal_curves$slope_1 <- rep(-999, nrow(cal_curves))
cal_curves$r2_1 <- rep(-999, nrow(cal_curves))

cal_curves$slope_dalk <- rep(-999, nrow(cal_curves))
cal_curves$r2_dalk <- rep(-999, nrow(cal_curves))

cal_curves$slope_man <- rep(-999, nrow(cal_curves))
cal_curves$r2_man <- rep(-999, nrow(cal_curves))
...
for(i in 1:nrow(cal_curves)){
  ...
  lm_temp_1 <- lm(Cal_amt_ng ~ 0 + vol_is_normnorm_1, data = subset)
  
  slope <- as.numeric(coefficients(lm_temp_1))
  cal_curves$slope_1[i] <- slope
  
  r2 <- summary(lm_temp_1)$r.squared
  cal_curves$r2_1[i] <- r2
  
  lm_temp_dalk <- lm(Cal_amt_ng ~ 0 + vol_is_normnorm_dalk, data = subset)
  
  slope <- as.numeric(coefficients(lm_temp_dalk))
  cal_curves$slope_dalk[i] <- slope
  
  r2 <- summary(lm_temp_dalk)$r.squared
  cal_curves$r2_dalk[i] <- r2
  
  if(is.na(subset$vol_is_normnorm_man[1])){
    next
  }
  else{
  
  lm_temp_man <- lm(Cal_amt_ng ~ 0 + vol_is_normnorm_man, data = subset)
  
  slope <- as.numeric(coefficients(lm_temp_man))
  cal_curves$slope_man[i] <- slope
  
  r2 <- summary(lm_temp_man)$r.squared
  cal_curves$r2_man[i] <- r2
  }
}
...
real_cc_withRI <- real_cc %>% 
  ...
  mutate(inverse_slope_1 = 1/slope_1) %>% 
  mutate(inverse_slope_dalk = 1/slope_dalk) %>% 
  mutate(inverse_slope_man = 1/slope_man)


write.csv(real_cc_withRI, file = "cal_factors4.csv")
...
```
Visualizing the different IS norm methods

```{r}
for(i in 1:nrow(Analyte_with_IS)){
  name.temp <- Analyte_with_IS$Compound.Name[i]
  
  group.temp <- real_cc_withRI %>% 
    filter(cpnd == name.temp)
  
  group.temp.long <- group.temp %>% 
    gather(IS_type, inverse_slope_long, inverse_slope:inverse_slope_man, factor_key=TRUE)
  
  tt <- group.temp.long %>% ggplot(aes(IS_type, inverse_slope_long, ymin = 0)) +
    geom_boxplot()+
    ggtitle(name.temp)
...
for(i in 1:nrow(Analyte_with_IS)){
  ...
  if(mean(group.temp$r2_man)<0){
    next
  }
  
  group.temp.long <- group.temp %>% 
    gather(IS_type, r2_long, c(r2, r2_1, r2_dalk, r2_man), factor_key=TRUE)
  
  tt <- group.temp.long %>% ggplot(aes(IS_type, r2_long, ymin = 0)) +
    geom_boxplot()+
    ggtitle(name.temp)
...
```
Summarizing the performance of the different methods
```{r}
ISnorm_comparison_summary <- real_cc_withRI %>% 
  group_by(cpnd) %>% 
  summarise(slope_stdv = sd(inverse_slope), slope_stdv_1 = sd(inverse_slope_1), slope_stdv_dalk = sd(inverse_slope_dalk), slope_stdv_man = sd(inverse_slope_man), mean_r2 = mean(r2), mean_r2_1 = mean(r2_1), mean_r2_dalk = mean(r2_dalk), mean_r2_man = mean(r2_man))


man_included_ISnorm_comp <- ISnorm_comparison_summary %>% 
  filter(mean_r2_man > 0)

man_included_ISnorm_comp %>% 
  gather(IS_type, slope_sd, slope_stdv:slope_stdv_man, factor_key = TRUE) %>% 
  ggplot(aes(IS_type, slope_sd))+
  geom_boxplot()+
  coord_cartesian(ylim=c(0, 20000))

man_included_ISnorm_comp %>% 
  gather(IS_type, mean_r2_all, mean_r2:mean_r2_man, factor_key = TRUE) %>% 
  ggplot(aes(IS_type, mean_r2_all))+
  geom_boxplot()

summary(man_included_ISnorm_comp$mean_r2)

summary(man_included_ISnorm_comp$mean_r2_man)

summary(man_included_ISnorm_comp$slope_stdv)

summary(man_included_ISnorm_comp$slope_stdv_man)
...
# use 88
name_with_cat <- cat %>% 
  dplyr::select(Name, Func_group_cat) %>% 
  rename(Compound.Name = Name)

comp_pop_1 <- compound_pop %>% 
  left_join(name_with_cat)

#sampling forcing the pct of the categories to be equal
set.seed(88)
tty<-sample.split(comp_pop_1$Func_group_cat, SplitRatio = 8/10)
...
slopemod.train.names = slopemod_temp[tty,]

slopemod.train <- slopemod.train.names %>% 
  rename(cpnd = Compound.Name) %>% 
  inner_join(cc_slopemod) 
  
slopemod.test.names = slopemod_temp %>% 
  anti_join(slopemod.train.names)
...
confirmation_train <- comp_pop_1[tty,]
confirmation_test <- comp_pop_1 %>% 
  anti_join(confirmation_train)

table(confirmation_train$Func_group_cat)
table(confirmation_test$Func_group_cat)
...
slopemod.train = slopemod.train.o %>% 
  ...
  select(-slope_dalk) %>% 
  select(-slope_man) %>% 
  mutate(Appropriate_cc = as.factor(dates)) %>% 
  select(-dates)
