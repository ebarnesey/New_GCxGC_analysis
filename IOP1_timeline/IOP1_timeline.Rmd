...
bt_sum <- read_csv("Blob_table_summary_JustSample.csv")

date <- bt_sum$File_num[10]
...
make_file_name <- function(date_string) {
  file_start <- "IOP1_blobtables/Copy of "
...
Reverse_match_factor_floor <- 700
LRI_diff_floor <- 10

fb_match_factor_floor <- 700
fb_reverse_match_factor_floor <- 600
...
IS_goodmatch <- M_t_LRI %>% 
  ...
  filter(Library.Match.Factor > Match_factor_floor || Library.Reverse.Match.Factor > Reverse_match_factor_floor | Compound.Name == "methoxyphenol-d3")  
...
IS_positions_12030456 <- read.csv("IOP1_blobtables/IS_1203_0456.csv")
...
IS_positions_avg <- IS_positions %>% 
  ...
  summarise(LRI_avg_all = mean(LRI_avg)) %>% 
  left_join(IS_rt2)
...
IS_stability %>% 
  ggplot(aes(x = dalk_norm, y = IS_mean_norm, color = Retention.II..sec.))+
  geom_point()+
  ylim(0, 2) +
  xlim(0,2)
...
LRI1_avg = mean(IS_positions_avg$LRI_avg_all)
LRI2_avg = mean(IS_positions_avg$Retention.II..sec.)

IS_positions_avg <- IS_positions_avg %>% 
  mutate(relative_LRI = LRI_avg_all/LRI1_avg) %>% 
  mutate(relative_LRI2 = Retention.II..sec./LRI2_avg)
...
testnn <- nn2(IS_pos_short, Analyte_pos_short, treetype = "kd", searchtype = "standard", k=1)
