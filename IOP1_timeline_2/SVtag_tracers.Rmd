...
p27 <- M_t_analyte_unique_IS %>% 
  filter(IOP == 2) %>% 
  filter(Compound.Name == "1221_0832_blob_42"| Compound.Name == "1221_0832_blob_35") %>% 
  left_join(just_bb2.m, by = "r_date") %>% 
  ggplot(aes(x = bb2.vol_is_rawnorm_ptnorm, y = vol_is_rawnorm_ptnorm, color = Compound.Name))+
  geom_point()+
  scale_color_discrete(name = "Monoterpene \nOxidation Products", labels = c("Unidentified Alpha Pinene\nOxidation Product", "Unidentified Limonene\nOxidation Product"))+
  ggtitle("Monoterpene Oxidation Products \nAssociated with BB Influence")+
  xlab("Normalized BB Tracer Signal")+
  ylab("Normalized Monoterpene \nOxidation Product Signal")+
  theme_bw()+
  theme(text = element_text(size = 14))
...
p29 <- M_t_analyte_unique_IS %>% 
  filter(IOP == 2) %>% 
  filter(Compound.Name == "1221_0832_blob_42") %>% 
  left_join(just_bb2.m, by = "r_date") %>% 
  ggplot(aes(x = bb2.vol_is_rawnorm_ptnorm, y = vol_is_rawnorm_ptnorm, color = Compound.Name))+
  geom_point()+
  scale_color_discrete(name = "Monoterpene \nOxidation Products", labels = c("Unidentified Alpha Pinene\nOxidation Product", "Unidentified Limonene\nOxidation Product"))+
...
cols <- c("Urban Plume Tracer \n(Phthalic Acid)\n*not to scale" = "deeppink4", "Biomass\n Burning Tracer\n(Levoglucosan)\n*not to scale"= "blue4")
...
p3 <- ggplot()+
  ...
  scale_fill_discrete(name="",
                         labels=c("In Prior Sample", "Not In Prior Sample"))+
  theme_bw()+
  guides(col = guide_legend(order = 2),fill = guide_legend(order = 1))+
  scale_x_datetime(date_breaks = "3 day", labels = date_format("%b %d"), limits = c(unique_tracing_c1.m$r_date[25], unique_tracing_c1.m$r_date[54]))+
...
test <- unique_tracing_c2 %>% 
  ...
  mutate(r_date.d = as.Date(r_date))
...
p5 <- ggplot()+
  # geom_col(data = IOP2_filtertimes_tod, aes(x = r_date, y = 15, fill = D_N))+
  # scale_fill_manual("Time Of Day", values = c("D" = "lightyellow2", "N" = "grey70"))+
  # labs("")+
  geom_col(data = unique_tracing_c2, aes(x = r_date, y = vol_is_rawnorm_ptnorm, fill = unique_log))+
  labs(fill = "Unique Compared \n To Prior Sample")+
  ...
  guides(col = guide_legend(order = 2),fill = guide_legend(order = 1))+
  ylim(0, 15)+
  scale_x_datetime(date_breaks = "3 day", labels = date_format("%b %d"), limits = c(unique_tracing_c2.m$r_date[31],unique_tracing_c2.m$r_date[108]))+
  theme(axis.text.x = element_text(angle = 25, vjust = 1.0, hjust = 1.0))
...
p6 <- unique_tracing_c1.m %>% 
  ...
  ggplot()+
  geom_point(aes(x = a1.vol_is_rawnorm_ptnorm/max(a1.vol_is_rawnorm_ptnorm), y = TRUE.), size = 2, shape = 17)+
  xlab("Normalized Urban Plume \nTracer Signal")+
  ylab("Normalized Organic Signal \n(Not in Prior Sample)")+
  annotate("text", x = .3, y = 1.5, label = "Urban Tracer \nr = .845", size = 7)+
  theme_bw()+
  theme(text = element_text(size = 18), axis.title.x=element_text(colour= "deeppink4"), axis.title.y=element_text(color= "lightseagreen"))
...
p10 <- unique_tracing_c1.m %>% 
  #filter(a1.vol_is_rawnorm_ptnorm >0) %>% 
  ggplot()+
  geom_point(aes(x = bb1.vol_is_rawnorm_ptnorm/max(bb1.vol_is_rawnorm_ptnorm), y = TRUE.), size = 2, shape = 17)+
  xlab("Normalized Biomass Burning \nTracer Signal")+
  ylab("Normalized Organic Signal \n(Not in Prior Sample)")+
  annotate("text", x = .3, y = 1.5, label = "Biomass Burning Tracer \nr = .344", size = 7)+
  theme_bw()+
  theme(text = element_text(size = 18), axis.title.x=element_text(colour= "blue4"), axis.title.y=element_text(color= "lightseagreen"))
...
p8 <- unique_tracing_c2.m %>% 
  #filter(a1.vol_is_rawnorm_ptnorm >0) %>% 
  ggplot()+
  geom_point(aes(x = a2.vol_is_rawnorm_ptnorm/max(a2.vol_is_rawnorm_ptnorm), y = TRUE.), size = 2, shape = 17)+
  xlab("Normalized Urban Plume \nTracer Signal")+
  ylab("Normalized Organic Signal \n(Not in Prior Sample)")+
  annotate("text", x = .3, y = 4, label = "Urban Tracer \nr = .093", size = 7)+
  theme_bw()+
  theme(text = element_text(size = 18), axis.title.x=element_text(colour= "deeppink4"), axis.title.y=element_text(color= "lightseagreen"))
...
p9 <- unique_tracing_c2.m %>% 
  #filter(a1.vol_is_rawnorm_ptnorm >0) %>% 
  ggplot()+
  geom_point(aes(x = bb2.vol_is_rawnorm_ptnorm/max(bb2.vol_is_rawnorm_ptnorm), y = TRUE.), size = 2, shape = 17)+
  xlab("Normalized Biomass Burning \nTracer Signal")+
  ylab("Normalized Organic Signal \n(Not in Prior Sample)")+
  annotate("text", x = .3, y = 4, label = "Biomass Burning \nTracer r = .693", size = 7)+
  theme_bw()+
  theme(text = element_text(size = 18), axis.title.x=element_text(color= "blue4"), axis.title.y=element_text(color= "lightseagreen"))
...
date_testing <- M_t_analyte_unique_IS %>% 
  group_by(r_date) %>% 
  summarise(vol_ptnorm = sum(Volume/Punch_num_sample_time_norm), volisnormptnorm = sum(vol_is_rawnorm_ptnorm), IOP = mean(IOP))

date_testing_season <- date_testing %>% 
  group_by(IOP) %>% 
  summarise(mean_vol = mean(vol_ptnorm), med_vol = median(vol_ptnorm))

date_testing %>% 
  ggplot(aes(x = r_date, y = vol_ptnorm))+
  geom_line()+
  geom_line(aes(x = r_date, y = volisnormptnorm*10000), color = "red")
