...
water_t <- read_csv("flume_water_temp.csv") %>% 
  mutate(r_date = mdy_hm(Datetime))
...
bt_sum_bloom3 <- bt_sum_bloom3 %>% mutate(r_date =    mdy_hm(SS_startdate)) %>% 
  ...
  mutate(water_temp = NA)

for(i in 1:(nrow(bt_sum_bloom3)-1)){
  t_start = bt_sum_bloom3$r_date[i]
  t_end = bt_sum_bloom3$r_date[i+1]
  
  time_temp <- water_t %>% 
    filter(r_date > t_start) %>% 
    filter(r_date < t_end)
  
  bt_sum_bloom3$water_temp[i]= mean(time_temp$Flume_T_C)
  
}

chlor.mean <- mean(bt_sum_bloom3$Extracted_Chla, na.rm = TRUE)
chlor.sd <- sd(bt_sum_bloom3$Extracted_Chla, na.rm = TRUE)
chlor.max <- max(bt_sum_bloom3$Extracted_Chla, na.rm = TRUE)

hb.mean <- mean(bt_sum_bloom3$Bulk_HB_updated, na.rm = TRUE)
hb.sd <- sd(bt_sum_bloom3$Bulk_HB_updated, na.rm = TRUE)
hb.max <- max(bt_sum_bloom3$Bulk_HB_updated, na.rm = TRUE)

t.mean <- mean(bt_sum_bloom3$water_temp, na.rm = TRUE)
t.sd <- sd(bt_sum_bloom3$water_temp, na.rm = TRUE)
t.max <- max(bt_sum_bloom3$water_temp, na.rm = TRUE)


bt_sum_bloom3 <- bt_sum_bloom3 %>% 
  mutate(chlor.z = (Extracted_Chla-chlor.mean)/chlor.sd) %>% 
  mutate(hb.z = (Bulk_HB_updated-hb.mean)/hb.sd) %>% 
  mutate(temp.z = (water_temp-t.mean)/t.sd) %>% 
  mutate(chlor.m = Extracted_Chla/chlor.max) %>% 
  mutate(hb.m = Bulk_HB_updated/hb.max) %>% 
  mutate(temp.m= water_temp/t.max)
...
M_t_analyte_summary <- M_t_all_analytes_volcount %>% 
  dplyr::left_join(M_t_all_analytes_ncount) %>% 
  dplyr::left_join(M_t_match_analytes_volcount) %>% 
  dplyr::left_join(M_t_match_analytes_ncount) %>% 
...
perct_comps_traced <- num_analyte_unique %>% 
   dplyr::left_join(M_t_analyte_count) %>% 
...
SeaScape_cumsum_raw <- M_t_analyte_unique %>% 
  ...
  dplyr::summarise(raw_cumsum = sum(Punch_t_norm_vol), num_obs = sum(tic)) %>% 
  dplyr::mutate(pct_obs = num_obs/nrow(bt_sum_bloom3)) %>% 
  dplyr::arrange(desc(raw_cumsum))
...
IS_avg_vols.s <- IS_unique %>% 
  dplyr::group_by(Compound.Name) %>% 
  dplyr::summarise(IS_avgvol = mean(Volume_tot), IS_medvol = median(Volume_tot))

write.csv(IS_avg_vols.s, "IS_mean_med_vols_SSAbloom3_samples.csv")
...
File_norm_totorg <- M_t_analyte_withIS %>% 
  dplyr::group_by(r_date) %>% 
  dplyr::summarise(vol_normnorm_total = sum(vol_is_normnorm_ptnorm), vol_rawnorm_total = sum(vol_is_rawnorm_ptnorm))
...
Making the wide version of the IS normalized timelines, 
```{r}

M_t_analyte_withIS_wide <- M_t_analyte_withIS %>% 
  arrange(desc(pct_of_samp)) %>% 
  dplyr::select(r_date, Compound.Name, vol_is_normnorm_ptnorm) %>% 
  spread(Compound.Name, vol_is_normnorm_ptnorm, fill = 0)

write.csv(M_t_analyte_withIS_wide, "Barnes_SeaScape_NascentSSA.csv")
...
Colors for btz plotting

```{r}
gg_color_btzshared <- function(n) {
  hues = seq(15, 375, length = n + 1)
  hcl(h = hues, l = 55, c = 100)[1:n]
}

n = 6
cols_btzshared = gg_color_btzshared(n)

dev.new(width = 4, height = 4)
plot(1:n, pch = 16, cex = 2, col = cols_btzshared)

gg_color_btzssa <- function(n) {
  hues = seq(15, 375, length = n + 1)
  hcl(h = hues, l = 75, c = 100)[1:n]
}

n = 9
cols_btzssa = gg_color_btzssa(n)

dev.new(width = 4, height = 4)
plot(1:n, pch = 16, cex = 2, col = cols_btzssa)


scico_palette_show()

imola <- scico(6, palette = 'imola')

dev.new(width = 4, height = 4)
plot(1:5, pch = 16, cex = 2, col = imola.dk)

buda <- scico(6, palette = 'buda')

dev.new(width = 4, height = 4)
plot(1:9, pch = 16, cex = 2, col = buda.lt)
...
imola.dk <- darken(imola, amount = .05)
imola.lt <- lighten(imola, amount = .2)

buda.lt <- lighten(scico(9, palette = 'buda'), amount = .2)
...
Quantifying benzothiazoles

```{r}
btz_info <- read_csv("Benzothiazoles_Combined_info_trimmed.csv")
btz_qf <- read_csv("btz_quantfactors.csv")

btz_ssa_list <- btz_info %>% 
  filter(!is.na(SSA_name)) %>% 
  dplyr::select(SSA_name, My_Name, Quant_Proxy) %>% 
  left_join(btz_qf) %>% 
  mutate(keep = 1)


# note: constants: flow rate = 22.9 lpm, total collection area corresponds to
flow_rate <- 22.9 #L/min
pun_per_f <- 22.536 
M_t_analyte_withIS.btz <- M_t_analyte_withIS %>% 
  left_join(btz_ssa_list, by = c("Compound.Name"= "SSA_name")) %>% 
  filter(keep ==1) %>% 
  mutate(ng_per_sample = vol_is_rawnorm/QF_mean) %>% 
  mutate(ng_per_cm3 = ng_per_sample*(1/Filter_punches)*(pun_per_f)*(1/(flow_rate*60000*Time_sampled_number*24)))

btz_voc <- read_csv("btz_voc.csv") %>% 
  mutate(Date_time = mdy_hm(Date_time))

M_t_analyte_withIS.btz %>% 
  ggplot(aes(x = r_date, y = ng_per_cm3, fill = My_Name)) +
  geom_col()

coeff <- 1e-5

pp_btz <- ggplot(M_t_analyte_withIS.btz) +
  
  geom_col(aes(x = r_date, y=(ng_per_cm3*1e6), fill = My_Name)) + 
  geom_point(data = btz_voc, aes(x = Date_time, y=(btz_ppt/1000) * coeff, color = "Benzothiazole - VOC")) + # Divide by 10 to get the same range than the temperature
  theme(legend.position = "bottom")+
  xlab("")+
  scale_y_continuous(
    
    # Features of the first axis
    name = "Aerosol Concentration (ng/m3)",
    
    # Add a second axis and specify its features
    sec.axis = sec_axis(~./coeff, name="Gas Phase (ppb)")
  )+
...
png("btz_trial1.png", width=9, height=7,
    units="in", res=600, pointsize=12)
pp_btz
dev.off()

max_chla <- max(bt_sum_bloom3$Extracted_Chla, na.rm = TRUE)
max_hb <- max(bt_sum_bloom3$Bulk_HB_updated, na.rm = TRUE)
chla_dat <- bt_sum_bloom3[!is.na(bt_sum_bloom3$Extracted_Chla),]
maxt <- max(bt_sum_bloom3$water_temp, na.rm = TRUE)


chlor_hb <- ggplot()+
  geom_line(data = bt_sum_bloom3[!is.na(bt_sum_bloom3$Extracted_Chla),], aes(x = r_date, y = Extracted_Chla/max_chla, color = "Chlorophyll-a"), size = 1.5)+
  geom_line(data = bt_sum_bloom3[!is.na(bt_sum_bloom3$Bulk_HB_updated),], aes(x = r_date, y = Bulk_HB_updated/max_hb, color = "Heterotrophic Bacteria"), size = 1.5)+
  scale_color_manual(values = c("green4", "blue"))+
  labs(color = "")+
  xlab("")+
  ylab("Concentration \n(Normalized to Max)")+
  theme_bw(base_size = 18)+
  theme(legend.title=element_blank(), legend.margin=margin(c(.5,.5,.5,.5)), axis.text.x = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), legend.position = c(0.25, 0.85))

chlor_hb

png("chlorhb1.png", width=10, height=3.5,
    units="in", res=600, pointsize=12)
chlor_hb
dev.off()

#note this is for the voc plotting
# chlor_hb2 <- ggplot()+
#   geom_line(data = bt_sum_bloom3[!is.na(bt_sum_bloom3$Extracted_Chla),], aes(x = r_date, y = Extracted_Chla/max_chla, color = "Chlorophyll-a"), size = 1.5)+
#   geom_line(data = bt_sum_bloom3[!is.na(bt_sum_bloom3$Bulk_HB_updated),], aes(x = r_date, y = Bulk_HB_updated/max_hb, color = "Heterotrophic Bacteria"), size = 1.5)+
#   scale_color_manual(values = c("green4", "blue"))+
#   labs(color = "")+
#   xlim(min(bt_sum_voc$r_date), max(bt_sum_voc$r_date))+
#   xlab("")+
#   ylab("Concentration \n(Normalized to Max)")+
#   theme_bw(base_size = 16)+
#   theme(legend.title=element_blank(), legend.margin=margin(c(.5,.5,.5,.5)), axis.text.x = element_blank(),
# panel.background = element_blank())
# 
# chlor_hb2

time_min_plot = min(bt_sum_bloom3$r_date)
time_max_plot = max(btz_voc$Date_time, na.rm = TRUE)

chlor_hb_wt <- ggplot()+
  geom_line(data = bt_sum_bloom3[!is.na(bt_sum_bloom3$Extracted_Chla),], aes(x = r_date, y = chlor.z, color = "Chlorophyll-a"), size = 1.5)+
  geom_line(data = bt_sum_bloom3[!is.na(bt_sum_bloom3$Bulk_HB_updated),], aes(x = r_date, y = hb.z, color = "Heterotrophic Bacteria"), size = 1.5)+
  geom_line(data = bt_sum_bloom3[!is.na(bt_sum_bloom3$water_temp),], aes(x = r_date, y = temp.z, color = "Water Temperature"), size = 1.5)+
  # scale_y_continuous(
  #   
  #   # Features of the first axis
  ...
  #   
  #   # Add a second axis and specify its features
  #   sec.axis = sec_axis((~.-23)/4, name="Water Temperature (Celcius)")
  # )+
  
  
  scale_color_manual(values = c("green4", "blue", "grey30"))+
  labs(color = "")+
  scale_x_datetime(date_breaks = "3 days", labels = date_format("%b %d"), limits = c(time_min_plot, time_max_plot))+
  xlab("")+
  ylab("Concentration and\n   Water Temp (Z-scored)")+
  theme_bw(base_size = 18)+
  theme(legend.title=element_blank(), axis.text.x = element_blank(), legend.margin=margin(c(.5,.5,.5,.5)), 
...
chlor_hb_wt2 <- ggplot()+
  ...
  scale_color_manual(values = c("green4", "blue", "grey50"))+
  labs(color = "")+
  scale_x_datetime(date_breaks = "3 days", labels = date_format("%b %d"), limits = c(time_min_plot, time_max_plot))+
  xlab("")+
  ylab("Concentration and\nWater Temp (Z-scored)")+
  theme_bw(base_size = 16)+
  theme(legend.title=element_blank(), legend.margin=margin(c(.5,.5,.5,.5)),
  #theme(legend.title=element_blank(), axis.text.x = element_blank(), legend.margin=margin(c(.5,.5,.5,.5)), 
panel.background = element_blank())
...
chlor_hb_wt3 <- ggplot()+
  geom_line(data = bt_sum_bloom3[!is.na(bt_sum_bloom3$Extracted_Chla),], aes(x = r_date, y = chlor.m, color = "Chlorophyll-a"), size = 1.5)+
  geom_line(data = bt_sum_bloom3[!is.na(bt_sum_bloom3$Bulk_HB_updated),], aes(x = r_date, y = hb.m, color = "Heterotrophic Bacteria"), size = 1.5)+
  geom_line(data = bt_sum_bloom3[!is.na(bt_sum_bloom3$water_temp),], aes(x = r_date, y = (water_temp-24)*.3, color = "Water Temperature"), size = 1.5)+
  scale_y_continuous(
    
    # Features of the first axis
    #name = "Aerosol Concentration \n(ng/m^3)",
    name = "Conc. in Seawater\n(Normalized to Max)",
    
    # Add a second axis and specify its features
    sec.axis = sec_axis(~.*(10/3)+24, name="Water Temp (C)")
  )+
  # scale_y_continuous(
  #   
  #   # Features of the first axis
...
btz_mean_conc <- M_t_analyte_withIS.btz %>% 
  dplyr::select(My_Name, r_date, ng_per_cm3) %>% 
  spread(My_Name, ng_per_cm3, fill = 0) %>% 
  gather(key = "My_Name", value = "ng_per_cm3", -r_date) %>% 
  dplyr::group_by(My_Name) %>% 
  dplyr::summarise(ng_per_cm3.mean = mean(ng_per_cm3),
            ng_per_m3.sd = sd(ng_per_cm3)*1e6) %>% 
  mutate(ng_per_m3.mean = ng_per_cm3.mean*1e6) 

btz_mean_conc <- btz_mean_conc%>% 
  mutate(vol_frac_ssa = ng_per_m3.mean/sum(btz_mean_conc$ng_per_m3.mean, na.rm = TRUE)) %>% 
  dplyr::arrange((vol_frac_ssa)) %>% 
  dplyr::mutate(id = as.factor(row_number()))

btz_plotcols <- btz_info %>% 
  dplyr::select(My_Name, Plot_color)

btz_plot_labs <- btz_mean_conc %>% 
  dplyr::select(My_Name, id) %>% 
  left_join(btz_plotcols)

#btzlabnames <- as.list(btz_plot_labs$My_Name)

M_t_analyte_withIS.btz <- M_t_analyte_withIS.btz %>% 
  left_join(btz_plot_labs)

#l <- c("a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n")

coeff2 <- .7

pp_btz2 <- ggplot(M_t_analyte_withIS.btz) +
  
  geom_col(aes(x = r_date, y=(ng_per_cm3*1e6), fill = id)) + 
  geom_point(data = btz_voc, aes(x = Date_time, y=(btz_ppt)*coeff2, color = "Benzothiazole - VOC"), size = 3) + # Divide by 10 to get the same range than the temperature
  #theme(legend.position = "bottom")+
  xlab("")+
  scale_y_continuous(
    ...
    # Features of the first axis
    #name = "Aerosol Concentration \n(ng/m^3)",
    name = expression(Aerosol~Conc.~(ng/m^3)),
    
    # Add a second axis and specify its features
    sec.axis = sec_axis(~./coeff2, name="Gas Phase Conc. (ppt)")
  )+
  scale_color_manual(values = c("black"))+
  scale_fill_manual(labels = btz_plot_labs$My_Name, values = btz_plot_labs$Plot_color)+
  labs(color = "", fill = "")+
  scale_x_datetime(date_breaks = "3 days", labels = date_format("%b %d"), limits = c(time_min_plot, time_max_plot))+
  #guides(fill=guide_legend(ncol=3))+
  theme_bw(base_size = 18)+
...
png("btz_trial3.png", width=11, height=7,
    units="in", res=600, pointsize=5)
pp_btz2
dev.off()

btztseries2 <- ggarrange(chlor_hb_wt3, pp_btz2, heights = c(9, 16), 
          labels = c("A", "B"),
          ncol = 1, nrow = 2, align = "v")


png("btz_tseries9.png", width=9.5, height=9.5,
    units="in", res=1000, pointsize=2)
btztseries2
dev.off() 

btz_mean_voc <- mean(btz_voc$btz_ppt, na.rm = TRUE)
  

btz_ssa_trace <- M_t_analyte_withIS.btz %>% 
  filter(Compound.Name == "20200803_1535_blob_958_") %>% 
  dplyr::select(r_date, ng_per_cm3)



pp_btz3 <- ggplot(M_t_analyte_withIS.btz) +
  
  geom_col(aes(x = r_date, y=(ng_per_cm3*1e6), fill = id), position = "fill") + 
  # geom_point(data = btz_voc, aes(x = Date_time, y=(btz_ppt)*coeff2, color = "Benzothiazole - VOC"), size = 3) + # Divide by 10 to get the same range than the temperature
  # #theme(legend.position = "bottom")+
  # xlab("")+
  # scale_y_continuous(
...
btztseries3 <- ggarrange(chlor_hb_wt3, pp_btz3, heights = c(9, 16), 
          labels = c("A", "B"),
          ncol = 1, nrow = 2, align = "v")


png("btz_tseriesstacked1.png", width=9.5, height=9.5,
    units="in", res=1000, pointsize=2)
btztseries3
dev.off() 
...
```
time series of each individual benzothiazole
```{r}

M_t_analyte_withIS.btz.zparam <- M_t_analyte_withIS.btz %>% 
  dplyr::group_by(Compound.Name) %>% 
  dplyr::summarise(ng_per_cm3.mean = mean(ng_per_cm3, na.rm = TRUE), ng_per_cm3.sd = sd(ng_per_cm3, na.rm = TRUE))

M_t_analyte_withIS.btz.z <- M_t_analyte_withIS.btz %>% 
  left_join(M_t_analyte_withIS.btz.zparam) %>% 
  mutate(ng_per_cm3_zscore = (ng_per_cm3-ng_per_cm3.mean)/ng_per_cm3.sd)



pp_btz_each <- ggplot(M_t_analyte_withIS.btz.z) +
  
  geom_line(aes(x = r_date, y=(ng_per_cm3_zscore), color = id), size = 1.5)+
  facet_grid(id ~ .)+
  scale_color_manual(labels = btz_plot_labs$My_Name, values = btz_plot_labs$Plot_color)+
  ylab("Aerosol Concentration (Z-scored)")+
  xlab("")+
  theme_bw(base_size = 16)+
  labs(color = "")
...
png("btz_tseries_indv4.png", width=6, height=9,
    units="in", res=1000, pointsize=2)
pp_btz_each
dev.off() 

btzpool_ssa <- M_t_analyte_withIS.btz %>% 
  dplyr::group_by(r_date) %>% 
  dplyr::summarise(ng_per_cm3.sum = sum(ng_per_cm3))

btzpool.mean = mean(btzpool_ssa$ng_per_cm3.sum)
btzpool.sd = sd(btzpool_ssa$ng_per_cm3.sum)

btzpool_ssa <- btzpool_ssa %>% 
  mutate(ng_per_cm3.sum.z = (ng_per_cm3.sum-btzpool.mean)/btzpool.sd)

silox.z <- M_t_analyte_withIS %>% 
  filter(Compound.Name == "20200803_1535_blob_3_fb") %>% 
  mutate(vol.z = (vol_is_rawnorm_ptnorm-mean(vol_is_rawnorm_ptnorm))/sd(vol_is_rawnorm_ptnorm)) %>% 
  dplyr::select(r_date, Compound.Name, vol_is_rawnorm_ptnorm, vol.z)

oiltrac.z <- M_t_analyte_withIS %>% 
  filter(Compound.Name == "20200803_1535_blob_33_oil") %>% 
  mutate(voloil.z = (vol_is_rawnorm_ptnorm-mean(vol_is_rawnorm_ptnorm))/sd(vol_is_rawnorm_ptnorm)) %>% 
  dplyr::select(r_date, Compound.Name, vol_is_rawnorm_ptnorm, voloil.z)
  

btzpool <- chlor_hb_wt2+
  geom_line(data = btzpool_ssa, aes(x = r_date, y = ng_per_cm3.sum.z), color = "red")+
  geom_point(data = btzpool_ssa, aes(x = r_date, y = ng_per_cm3.sum.z, fill = "Benzothiazole Carbon Pool"), color = "red", shape = 23, size = 2)+
  geom_line(data = silox.z, aes(x = r_date, y = vol.z), color = "orange")+
  geom_point(data = silox.z, aes(x = r_date, y = vol.z, 
              fill = "Cycloheptasiloxane, \ntetradecamethyl-"), shape = 22,
             color = "orange", size = 2)+
  geom_line(data = oiltrac.z, aes(x = r_date, y = voloil.z), color = "purple")+
  scale_fill_manual(values = c("red", "orange"))

btzpool

siloxbtz <- silox.z %>% 
  left_join(btzpool_ssa) %>% 
  left_join(oiltrac.z)

siloxbtz %>% 
  ggplot(aes(x = vol.z, y = ng_per_cm3.sum.z))+
  geom_point()



png("btz_pool2.png", width=8, height=5,
    units="in", res=1000, pointsize=2)
btzpool
dev.off() 
...
RSQUARE(siloxbtz$vol.z, siloxbtz$ng_per_cm3.sum)

```

```{r}
me.th.btz <- M_t_analyte_withIS.btz %>% 
  filter(Compound.Name == "20200803_1535_blob_764_") %>% 
  dplyr::select(Compound.Name, r_date, Volume, 
                IS_appropriate1, IS_appropriate2, IS_appropriate3, IS_vol1,
                IS_vol2, IS_vol3, IS_vol, vol_is_rawnorm, vol_rawnorm_total,
                QF_mean,
                ng_per_sample, ng_per_cm3)
...
SeaScape_normalized_cumsum <- M_t_analyte_withIS %>% 
  ...
  dplyr::summarise(totorgnorm_cumsum = sum(vol_is_normnorm_ptnorm_totorgnorm), isnormnormptnorm_cumsum = sum(vol_is_normnorm_ptnorm))
...
For Crocker- isotope correlations

```{r}
boxplot(SeaScape_normalized_cumsum$isnormnormptnorm_cumsum)
summary(SeaScape_normalized_cumsum$isnormnormptnorm_cumsum)


just_date_isot <- bt_sum_bloom3 %>% 
  dplyr::select(r_date, Sub_iso_z) %>% 
  mutate(Sub_iso_z = -1*Sub_iso_z)



just_over_20000 <- M_t_analyte_withIS %>% 
  left_join(SeaScape_normalized_cumsum, by = "Compound.Name") %>% 
  filter(isnormnormptnorm_cumsum > 20000) %>% 
  dplyr::select(Compound.Name, r_date, vol_is_rawnorm_ptnorm) %>% 
  spread(Compound.Name, vol_is_rawnorm_ptnorm, fill = 0) %>% 
  left_join(just_date_isot) %>% 
  dplyr::select(-r_date)



cov_isot <- data.frame(cor(just_over_20000, use ="pairwise.complete.obs"))

write.csv(cov_isot, "Isotope_look.csv")

M_t_analyte_withIS_isot <- M_t_analyte_withIS %>% 
  dplyr::filter(Compound.Name == "20200803_1535_blob_521_oil" 
         | Compound.Name == "20200803_1535_blob_21_"| Compound.Name =="20200803_1535_blob_598_oil"|Compound.Name == "20200803_0949_blob_18_")

M_t_analyte_withIS_isot %>% 
  filter(Compound.Name == "20200803_1535_blob_521_oil") %>% 
  ggplot(aes(x = r_date, y = vol_is_normnorm_ptnorm_totorgnorm, color = Compound.Name))+
  geom_point()+
  geom_point(data = bt_sum_bloom3, aes(x = r_date, y = Sub_iso_z * .002), color = "black")

M_t_analyte_withIS_isot %>% 
  filter(Compound.Name == "20200803_1535_blob_521_oil") %>% 
  ggplot(aes(x = Sub_iso_z, y = vol_is_rawnorm_ptnorm, color = Compound.Name))+
  geom_point()
  
M_t_analyte_withIS_isot %>% 
  filter(Compound.Name == "20200803_1535_blob_21_") %>% 
  ggplot(aes(x = Sub_iso_z, y = vol_is_rawnorm_ptnorm, color = Compound.Name))+
  geom_point()
  
M_t_analyte_withIS_isot %>% 
  filter(Compound.Name == "20200803_1535_blob_598_oil") %>% 
  ggplot(aes(x = Sub_iso_z, y = vol_is_rawnorm_ptnorm, color = Compound.Name))+
  geom_point()

M_t_analyte_withIS_isot %>% 
  filter(Compound.Name == "20200803_0949_blob_18_") %>% 
  ggplot(aes(x = Sub_iso_z, y = vol_is_rawnorm_ptnorm, color = Compound.Name))+
  geom_point()
