...
extralabs.ssa <- data.frame(id = as.factor(c(17,18)), My_Name = c("Both Phases", "SSA Only"), Plot_color = c("grey20", "grey80"), vol_frac_ssa = both.t$vol_frac_ssa) %>% 
...
extralabs.mdom <- data.frame(id = as.factor(c(17,19)), My_Name = c("Both Phases", "DOM Only"), Plot_color = c("grey20", "grey80"), vol_frac_mdom = both.tt$vol_frac_mdom) %>% 
  mutate(group = "foundwhere") %>% 
  mutate(width = 1)
...
btz.stickchart %>% 
  ...
  ggplot()+
  geom_point(aes(x = My_Name, y = vol_frac_ssa, fill = "SSA", color = Plot_color), size = 2, shape = 22)+
  geom_point(aes(x = My_Name, y = vol_frac_mdom, fill = "MDOM", color = Plot_color), size = 2, shape = 25)+
  scale_fill_manual(values = c("black", "white"), labels = c("SSA", "DOM"))+
  scale_color_manual(values = colorvals, labels = colornames)+
  
  geom_point(aes(x = My_Name, y = log(water_sol_mg_L)*coeff3), color = "lightblue3")+
  scale_y_continuous(
